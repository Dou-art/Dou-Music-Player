...
async function showAddToPlaylistModal(track) {
    console.log('showAddToPlaylistModal called with:', track);
    ...
    try {
        ...
        console.log('About to load playlists...');
        // 加载歌单列表
        await loadPlaylistsToModal(track.id);
        console.log('Playlists loaded');
    } catch (error) {
        console.error('Error in showAddToPlaylistModal:', error);
    }
}

// 加载用户歌单列表到弹窗
async function loadPlaylistsToModal(trackId) {
    console.log('loadPlaylistsToModal called, trackId:', trackId);
    const container = document.getElementById('playlistList');
    console.log('playlistList container:', container);
    if (!container) return;

    try {
        const res = await api.getMyPlaylists();
        console.log('getMyPlaylists response:', res);
        ...
        if (res?.playlist) {
            ...
            // 绑定点击事件
            const items = container.querySelectorAll('.playlist-list-item');
            console.log('Found playlist items:', items.length);
            items.forEach(item => {
                console.log('Binding click to:', item.dataset.playlistId, item.dataset.trackId);
                item.addEventListener('click', (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    console.log('Clicked playlist item:', item.dataset.playlistId, item.dataset.trackId);
                    addTrackToPlaylist(item.dataset.playlistId, item.dataset.trackId, item);
                });
            });
        }
    } catch (error) {
...
async function addTrackToPlaylist(playlistId, trackId, itemEl) {
    ...
    player.showToast('正在添加...');

    try {
        const res = await api.updatePlaylistTracks(playlistId, trackId, 'add');
        console.log('Add to playlist response:', res);
        
        // 检查各种可能的成功响应格式
        const code = res?.body?.code ?? res?.code;
        const message = res?.body?.message ?? res?.message ?? res?.msg;
        ...
        if (code === 200) {
            ...
            player.showToast('操作过于频繁，请稍后再试');
        } else {
            player.showToast(message || `添加失败 (code: ${code})`);
        }
    } catch (error) {
