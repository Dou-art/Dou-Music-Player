...
const api = {
    ...
    // 提取 cookie 对象（API 需要的格式）
    _getCookieObject() {
        const cookie = localStorage.getItem('music_cookie');
        if (!cookie) return null;
        const match = cookie.match(/MUSIC_U=([^;]+)/);
        return match ? { MUSIC_U: match[1] } : null;
    },

    // 带 cookie 的 POST 请求（通过 JSON body 传递 cookie）
    async requestWithCookie(endpoint, params = {}) {
        const urlStr = `${endpoint}?timestamp=${Date.now()}&randomCNIP=true`;
        
        // cookie 通过 body 传递，需要是对象格式
        const cookieObj = this._getCookieObject();
        const bodyParams = { ...params };
        if (cookieObj) {
            bodyParams.cookie = cookieObj;
        }
        ...
        try {
            ...
            console.error('API request with cookie failed:', error);
            throw error;
        }
    },
    ...
    async createPlaylist(name, privacy = false, type = 'NORMAL') {
        const cookie = this._getCookieObject();
        if (!cookie) {
            return { code: -1, message: '请先登录' };
        }
        return this.requestWithCookie('/playlist/create', { 
            name, 
    ...
    async deletePlaylist(id) {
        ...
        return this.requestWithCookie('/playlist/delete', { id });
    },
    ...
    async updatePlaylist(id, name, desc = '', tags = '') {
        const cookie = this._getCookieObject();
        if (!cookie) return { code: -1, message: '请先登录' };
        return this.requestWithCookie('/playlist/update', { id, name, desc, tags });
    },
    ...
    async updatePlaylistTracks(pid, trackIds, op = 'add') {
        ...
        return this.requestWithCookie('/playlist/tracks', { op, pid, tracks });
    },
