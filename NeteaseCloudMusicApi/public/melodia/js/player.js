class MusicPlayer {
    ...
    updateYrcWordHighlight(currentTime, lineIndex) {
        ...
        words.forEach(wordEl => {
            ...
            if (currentTime >= wordTime) {
                const progress = Math.min(1, (currentTime - wordTime) / wordDuration);
                wordEl.classList.add('yrc-active');
                wordEl.style.setProperty('--yrc-progress', progress);
            } else {
                wordEl.classList.remove('yrc-active');
                wordEl.style.removeProperty('--yrc-progress');
            }
        });
    }
    ...
    highlightCurrentLyric() {
        ...
        if (activeLine) {
            ...
            this.smoothScrollLyrics(scroll, targetScroll, 600);
        }
    }
